...
def _get_client() -> tuple[OpenAI, str]:
    """根据当前配置返回主服务商 (client, model_name)"""
    cfg = get_config()
    mode = cfg["model_mode"]
    if mode == "online":
        ocfg = cfg.get("vlm") or cfg["online"]
...
def _call_vlm_api(messages: list, max_tokens: int, temperature: float = 0.1) -> str:
    """
    统一 VLM 调用入口：先用主服务商，失败后自动切换备选服务商（若已启用）。
    返回模型回复的原始文本。
    """
    ...
    try:
        ...
        cfg = get_config()
        fb = cfg.get("vlm_fallback", {})
        if fb.get("enabled") and fb.get("api_key") and fb.get("base_url") and fb.get("model"):
            try:
                fb_client = OpenAI(api_key=fb["api_key"], base_url=fb["base_url"])
                resp = fb_client.chat.completions.create(
                    model=fb["model"], messages=messages,
                    max_tokens=max_tokens, temperature=temperature,
                )
                return resp.choices[0].message.content.strip()
            except Exception:
...
def analyze_page_layout(image_path: str, progress_cb=None) -> dict:
    ...
    raw = _call_vlm_api(messages, max_tokens=4096)
...
def table_image_to_markdown(image_path: str) -> str:
    ...
    return _call_vlm_api(messages, max_tokens=2048)
...
def formula_image_to_latex(image_path: str) -> str:
    ...
    return _call_vlm_api(messages, max_tokens=512)
